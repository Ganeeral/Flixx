...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['preview']) && isset($_FILES['video'])) {
        ...
        $sql = "INSERT INTO PendingVideos (title, description, author, publication_date, preview, video_url) VALUES ('$title', '$description', '$author', '$publication_date', '$preview', '$video_url')";

        if ($conn->query($sql) === TRUE) {
            echo json_encode(["message" => "Видео успешно добавлено на проверку"]);
        } else {
            echo json_encode(["error" => "Ошибка при добавлении видео на проверку: " . $conn->error]);
        }
